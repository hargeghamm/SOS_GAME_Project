...
class GameBoard:
    ...
    def print_game_board(self) -> None:
        for row in self._board:
            for i, space in enumerate(row):
                print(color_yellow + "|" if i == 0 else color_yellow + "", space + color_yellow + " |", end="")
    ...
    def apply_move(self, move: Move) -> None:
        if self.is_in_range(move.location):
            self._board[move.location.x][move.location.y] = move.sign

    def evaluate(self) -> int:
        # Implement a method to evaluate the board state
        # For simplicity, you might use the get_sos_count method
        return self.get_sos_count()

    def is_sos_created_by_move(self, move: Move) -> bool:
        # Implement logic to check if the move creates an SOS
        # This method should simulate applying the move and check for new SOS formations
        temp_board = self.clone()
        temp_board.apply_move(move)
        return temp_board.get_sos_count() > self.get_sos_count()
